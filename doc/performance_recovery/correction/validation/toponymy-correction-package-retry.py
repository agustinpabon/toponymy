...
with tempfile.TemporaryDirectory(prefix="toponymy-correction-package-") as tmp:
    ...
    with (out / "package-build-retry.txt").open("x") as log:
    ...
    with (out / "package-smoke-retry.txt").open("x") as log:
...
import os,tempfile
assert Path(toponymy.__file__).resolve().is_relative_to(Path(os.environ['PYTHONPATH']).resolve())
